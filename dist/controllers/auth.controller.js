...
async function Signup(userData) {
    ...
    // await user.save();
    await sendEmail(user.email, "Verify Your SyncCode Account", `
...
async function Signin(userData) {
    try {
        ...
        const passwordCheck = bcrypt.compareSync(userData.password, userExists.password);
        if (!passwordCheck) {
            return null;
        }
        const signedData = await generateToken({
...
export const sendOtp = async (req, res) => {
    ...
    await user.save();
    await sendEmail(user.email, "SyncCode Password Reset OTP", `
...
export const verifyOtp = async (req, res) => {
    ...
    const resetToken = generateToken({ userId: user._id }, "10m");
    user.otp = undefined;
    user.otpExpire = undefined;
    await user.save();
